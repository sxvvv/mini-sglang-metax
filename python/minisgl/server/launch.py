...
# 每个TP rank的Scheduler子进程入口
# Scheduler构造阶段会继续初始化Engine、模型、KV Cache和设备
...
def launch_server(run_shell: bool = False) -> None:
    ...
    server_args, run_shell = parse_args(sys.argv[1:], run_shell) # 解析参数
    logger = init_logger(__name__, "initializer")

    # API前端建立ZMQ通道，会调用这个回调启动全部后端进程
    def start_subprocess() -> None:
        import multiprocessing as mp

        from minisgl.tokenizer import tokenize_worker

        mp.set_start_method("spawn", force=True)  # spawn创建全新的python进程
        ...
        # world_size: GPU卡数，TP的并行度
        for i in range(world_size): # 每个TP rank创建一个 Scheduler
            new_args = replace(
                server_args,
        ...
        # Tensor Parallel使用“一张卡/一个 rank/一个 Scheduler进程”。
        # 每个进程获得相同配置，但tp_info中的rank不同。
