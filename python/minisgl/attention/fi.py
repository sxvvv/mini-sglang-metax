...
def _supported_optional_kwargs(callable_obj, **kwargs):
    """Keep optional FlashInfer kwargs exposed by the installed API."""
    try:
...
def _optional_backend_kwarg(wrapper_type: type, backend: str) -> Dict[str, str]:
    return _supported_optional_kwargs(wrapper_type, backend=backend)
