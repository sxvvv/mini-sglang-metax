"""Gate 4.12 Qwen3-1.7B TP=2 dynamic admission B: 1 → 2 → 1 smoke driver.

Runs a Qwen3-1.7B TP=2 dynamic-admission end-to-end on 2 x Ascend
...
Staggered admission strategy (script-local, no runtime change):
    ...
    This is exactly the pattern proven at Gate 4.5 on Qwen3-0.6B,
    now applied to Qwen3-1.7B.
...
Structured log format (per rank):
    ...
    ``GATE4.12_JSONL rank=<r> ...`` with the fields listed in
    Gate 4.12 spec §4, including:
      ...
      * ``cold_start_attempt_id`` / ``memory_sync_retry_note`` —
        Gate 4.10 pattern threaded through the outer shell loop

Exit code (rank 0):
    0 on PASS, 1 on FAIL, 2 on BLOCKED. Rank 1 mirrors via the CPU
    barrier in Scheduler.shutdown.

Out of scope (per Gate 4.12 spec §5):
    B=3, B: 1→2→3→2→1, TP=4, timing benchmark, chunked prefill,
    exception recovery, long soak, non-Qwen3 / non-1.7B models.
"""
...
class RankLog:
    ...
    def emit(self) -> None:
        ...
        print(
            f"GATE4.12_JSONL rank={self.rank} {json.dumps(payload, ensure_ascii=False)}",
            flush=True,
        )


def _err(rank: int, msg: str) -> None:
    print(f"[gate4.12][rank={rank}] {msg}", flush=True)
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Gate 4.12 Qwen3-1.7B TP=2 dynamic admission B: 1→2→1",
    )
    parser.add_argument(
        "--model-path",
...
def _run_worker(args: argparse.Namespace) -> int:
    ...
    if rank_env is None or world_size_env is None or local_rank_env is None:
        print(
            "[gate4.12] LOCAL_RANK / RANK / WORLD_SIZE must be set by the "
    ...
    if world_size != 2:
        ...
        log.failure_trace_summary = (
            f"Gate 4.12 pins world_size=2; got {world_size}"
        )
        log.status = "BLOCKED"
        log.emit()
        return 2
    ...
    try:
        ...
        if not (admission_ok and timeline_ok and joint_ok):
            raise RuntimeError(
                "Gate 4.12 dynamic admission invariants not met: "
