"""Gate 5.6 Qwen3-4B TP=2 timing baseline smoke driver.
...
Envelope (locked at Gate 5.6):
    ...
    model_path = /mnt/nvme/models/Qwen3-4B (default)
    use_pynccl = False (HCCL + gloo sidecar)

Timing semantics:
    * ``t0`` -- ``time.perf_counter()`` immediately before the
      offline run_forever call for the repeat
...
Case F reuses the Gate 5.5 dynamic-admission pattern: script-local
...
Structured log format (per rank):
    * one ``GATE5.6_JSONL rank=<r> {...}`` line per (case, warmup)
      and per (case, repeat_id) -- raw measurement records
    * one ``GATE5.6_SUMMARY rank=<r> {...}`` line per case -- the
      median / min / max summary
    * one final ``GATE5.6_TIMING_RESULT=<PASS|PARTIAL|BLOCKED>``
...
class RepeatRecord:
    ...
    def emit(self) -> None:
        ...
        print(
            f"GATE5.6_JSONL rank={self.rank} {json.dumps(payload, ensure_ascii=False)}",
            flush=True,
        )


def _err(rank: int, msg: str) -> None:
    print(f"[gate5.6][rank={rank}] {msg}", flush=True)
...
def _run_dynamic_admission_repeat(
    ...
    alone. Direct port of Gate 5.5 dynamic-admission driver.
    """
    sps = [
        sampling_params_cls(
            temperature=0.0,
...
def _emit_summary(
    ...
    print(
        f"GATE5.6_SUMMARY rank={rank} {json.dumps(payload, ensure_ascii=False)}",
        flush=True,
    )
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Gate 5.6 Qwen3-4B TP=2 timing baseline",
    )
    parser.add_argument("--model-path", default="/mnt/nvme/models/Qwen3-4B")
...
def _run_worker(args: argparse.Namespace) -> int:
    ...
    if rank_env is None or world_size_env is None or local_rank_env is None:
        print(
            "[gate5.6] LOCAL_RANK / RANK / WORLD_SIZE must be set by "
    ...
    if world_size != 2:
        _err(rank, f"Gate 5.6 pins world_size=2; got {world_size}")
        if rank == 0:
            print("GATE5.6_TIMING_RESULT=BLOCKED", flush=True)
        return 2
    ...
    if rank == 0:
        print(f"GATE5.6_TIMING_RESULT={result_flag}", flush=True)
