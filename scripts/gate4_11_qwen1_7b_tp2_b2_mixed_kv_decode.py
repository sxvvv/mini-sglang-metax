"""Gate 4.11 Qwen3-1.7B TP=2 B=2 mixed-KV decode explicit-evidence smoke driver.

Runs a Qwen3-1.7B TP=2 B=2 mixed-KV decode end-to-end on 2 x Ascend
910B1 and captures per-forward-pass ``FIAMetadata`` snapshots so the
mixed-KV decode invariant is visible in the structured log:

Cases:
    A. TP=2 init
    B. Qwen3-1.7B TP=2 model load
    C. B=2 unequal-length prompts (short + long), max_new_tokens=8
    D. mixed-KV decode metadata evidence: >=1 decode step with
       query_lengths == [1, 1] and kv_lengths[0] != kv_lengths[1]

Envelope (locked at Gate 4.11):
...
Metadata capture strategy:
    ...
    forward pass — once for prefill and once per decode step. The
    driver wraps that method (script-local monkey-patch) to record
    ``batch_size``, ``query_seq_lens`` and ``kv_seq_lens`` from the
    freshly-assigned ``FIAMetadata`` into an in-memory ring. The
    wrapper is installed BEFORE ``LLM()`` so any pre-generate
    forwards (e.g. warmup) are captured too; the driver slices the
    snapshots that fall inside the generate() call to isolate the
    B=2 batch. This is the same pattern proven at Gate 4.4 on
    Qwen3-0.6B; the runtime source under ``python/minisgl/`` is not
    modified.

Design:
    * Same per-rank worker shape as Gate 4.1 / 4.3 / 4.4 / 4.8 / 4.9 /
      4.10. torchrun x2, each rank reads LOCAL_RANK / RANK /
      WORLD_SIZE, use_pynccl=False.
    * Two prompts with different tokenized lengths (short + long) are
      enqueued once. Because the two uids start with different
      cached_len (both 0 at prefill, then diverge as prefill emits
      the last-token logits and each uid appends its own decode
      tokens), every decode-step KV length pair differs by exactly
      ``long_prompt_len - short_prompt_len`` — the mixed-KV
      invariant this gate exists to prove.
    * Only one generate() call per process — the Gate 2.2f-documented
      "ragged + non-zero cached_len + extend_len > 1" branch is not
      exercised.
    * ``cold_start_attempt_id`` and ``memory_sync_retry_note`` are
      threaded through the JSONL to reflect the launch's
      attempt-in-a-shell-loop index (Gate 4.10 pattern). The driver
      does NOT retry — the outer shell loop is the authorised
      recovery for transient ``_sync_get_memory()`` imbalance.

Structured log format (per rank):
    A single JSON object on stdout tagged ``GATE4.11_JSONL rank=<r> ...``
    with the fields listed in Gate 4.11 spec §4, including
    ``decode_step_snapshots`` (steps with ``query_lengths == [1, 1]``)
    and ``all_step_snapshots`` (full per-forward-pass trace for
    cross-check, including prefill).

Exit code (rank 0):
    0 on PASS, 1 on FAIL, 2 on BLOCKED. Rank 1 mirrors via the CPU
    barrier in Scheduler.shutdown.

This script does not do dynamic admission, does not do B > 2, does
not touch TP=4 or timing, does not benchmark, does not modify
...
class RankLog:
    ...
    def emit(self) -> None:
        ...
        print(
            f"GATE4.11_JSONL rank={self.rank} {json.dumps(payload, ensure_ascii=False)}",
            flush=True,
        )


def _err(rank: int, msg: str) -> None:
    print(f"[gate4.11][rank={rank}] {msg}", flush=True)
...
def _install_metadata_snapshot_hook(snapshots: List[StepSnapshot]) -> None:
    ...
    Gate 4.11 needs explicit decode-step evidence: after each forward
    ...
    pattern as Gate 4.4, now on Qwen3-1.7B.
    """
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Gate 4.11 Qwen3-1.7B TP=2 B=2 mixed-KV decode explicit evidence",
    )
    parser.add_argument(
        "--model-path",
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="Fixed at 8 for Gate 4.11; exposed only for debugging",
    )
    parser.add_argument(
        "--memory-ratio",
        type=float,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "--prompt-short so kv_seq_lens diverge on every decode step."
        ),
    )
    parser.add_argument(
...
def _run_worker(args: argparse.Namespace) -> int:
    ...
    if rank_env is None or world_size_env is None or local_rank_env is None:
        print(
            "[gate4.11] LOCAL_RANK / RANK / WORLD_SIZE must be set by the "
    ...
    if world_size != 2:
        ...
        log.failure_trace_summary = (
            f"Gate 4.11 pins world_size=2; got {world_size}"
        )
        log.status = "BLOCKED"
        log.emit()
        return 2
    ...
    # ------------------------------------------------------------------
    # A. Init + B. Model load (share one LLM boot)
    # ------------------------------------------------------------------
    llm = None
    ...
    try:
        ...
        # Install the metadata-snapshot hook BEFORE LLM() so any pre-
        # generate forwards (warmup / weight-load probe) are also
        # captured; the driver slices the generate() window later.
    ...
    try:
        ...
        if tokenized_lengths[0] == tokenized_lengths[1]:
            raise RuntimeError(
                f"Gate 4.11 requires unequal-length prompts; got "
        ...
        if len(mixed_kv_snaps) == 0:
            ...
            raise RuntimeError(
                "Gate 4.11 requires >=1 decode step with query_lengths=[1,1] "
