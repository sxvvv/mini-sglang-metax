"""Gate 5.3 Qwen3-4B TP=2 B=2 ragged prefill smoke driver.

Runs a Qwen3-4B TP=2 B=2 ragged-prefill (unequal-length prompts)
end-to-end on 2 x Ascend 910B1:
    A. TP=2 init
    B. Qwen3-4B TP=2 model load
    C. B=2 ragged prefill + decode, max_new_tokens=8

Envelope (locked at Gate 5.3):
...
FIA branch selection:
    Two prompts with DIFFERENT tokenized lengths, batched in one
    generate() call. Both uids arrive with cached_len == 0 (fresh
    boot, no radix prefix). This drives the FIA "ragged +
    cached_len == 0" prefill path (Gate 4.3 on Qwen3-0.6B, Gate
    4.10 on Qwen3-1.7B), now on Qwen3-4B. Mixed-KV decode and
    dynamic admission are NOT exercised. The Gate 2.2f documented
    "ragged + non-zero cached_len + extend_len > 1" unsupported
    branch is intentionally avoided.

Structured log format (per rank):
    A single JSON object on stdout tagged ``GATE5.3_JSONL rank=<r> ...``
    with the fields listed in Gate 5.3 spec section 4.

Footer (rank 0 only):
    ``GATE5.3_RESULT=<PASS|PARTIAL|BLOCKED>``

Exit code (rank 0):
    0 on PASS, 1 on FAIL, 2 on BLOCKED. Rank 1 mirrors via the CPU
    barrier in Scheduler.shutdown.

This driver does not do mixed-KV decode, does not do dynamic
admission, does not do B > 2, does not do timing, does not touch
TP > 2, does not benchmark, does not compare against SGLang / vLLM
/ TGI, does not modify python/minisgl/, does not modify tests.
"""
...
class RankLog:
    ...
    def emit(self) -> None:
        ...
        print(
            f"GATE5.3_JSONL rank={self.rank} {json.dumps(payload, ensure_ascii=False)}",
            flush=True,
        )


def _err(rank: int, msg: str) -> None:
    print(f"[gate5.3][rank={rank}] {msg}", flush=True)
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Gate 5.3 Qwen3-4B TP=2 B=2 ragged prefill",
    )
    parser.add_argument(
        "--model-path",
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="Fixed at 8 for Gate 5.3; exposed only for debugging",
    )
    parser.add_argument(
        "--memory-ratio",
        type=float,
...
def _run_worker(args: argparse.Namespace) -> int:
    ...
    if rank_env is None or world_size_env is None or local_rank_env is None:
        print(
            "[gate5.3] LOCAL_RANK / RANK / WORLD_SIZE must be set by the "
    ...
    if world_size != 2:
        ...
        log.failure_reason = f"Gate 5.3 pins world_size=2; got {world_size}"
        log.status = "BLOCKED"
        log.emit()
        if rank == 0:
            print("GATE5.3_RESULT=BLOCKED", flush=True)
        return 2
    ...
    try:
        ...
        if tokenized_lengths[0] == tokenized_lengths[1]:
            raise RuntimeError(
                f"Gate 5.3 requires unequal-length prompts (ragged); got "
        ...
        log.emit()
        if rank == 0:
            print("GATE5.3_RESULT=PARTIAL", flush=True)
        return 1
    ...
    if rank == 0:
        if log.status == "PASS":
            print("GATE5.3_RESULT=PASS", flush=True)
