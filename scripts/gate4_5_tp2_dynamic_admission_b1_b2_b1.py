"""Gate 4.5 TP=2 dynamic admission B: 1 → 2 → 1 smoke driver.

Runs a Qwen3-0.6B TP=2 dynamic-admission end-to-end on Ascend 910B1
and captures per-forward-step ``FIAMetadata`` snapshots so the batch
timeline ``[1, ..., 2, ..., 1]`` is directly visible in the
structured log:
...
Metadata capture strategy:
    ...
    admission trigger (it flips ``_ready_to_admit_b`` when it sees
    ``batch_size == 1 and query_lengths == [1] and active_uid == 0``).

Structured log format (per rank):
    A single JSON object on stdout tagged
    ``GATE4.5_JSONL rank=<r> ...`` with the fields listed in Gate 4.5
    spec §4, including:
...
Out of scope (per Gate 4.5 spec §5):
    B=3, B: 1→2→3→2→1, TP=4, Qwen3-1.7B TP=2, timing benchmark,
    chunked prefill, exception recovery, long soak.
"""
...
class RankLog:
    ...
    def emit(self) -> None:
        ...
        print(
            f"GATE4.5_JSONL rank={self.rank} {json.dumps(payload, ensure_ascii=False)}",
            flush=True,
        )


def _err(rank: int, msg: str) -> None:
    print(f"[gate4.5][rank={rank}] {msg}", flush=True)
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Gate 4.5 TP=2 dynamic admission B: 1→2→1 on Qwen3-0.6B",
    )
    parser.add_argument(
        "--model-path",
...
def _run_worker(args: argparse.Namespace) -> int:
    ...
    if rank_env is None or world_size_env is None or local_rank_env is None:
        print(
            "[gate4.5] LOCAL_RANK / RANK / WORLD_SIZE must be set by the "
    ...
    if world_size != 2:
        ...
        log.failure_trace_summary = f"Gate 4.5 pins world_size=2; got {world_size}"
        log.status = "BLOCKED"
        log.emit()
        return 2
    ...
    # Discard any pre-generate() snapshots (empty batches never reach FIA,
    ...
    try:
        ...
        joint_ok = log.joint_decode_step_count is not None and log.joint_decode_step_count >= 1
        ...
        if not (admission_ok and timeline_ok and joint_ok):
            raise RuntimeError(
                "Gate 4.5 dynamic admission invariants not met: "
